...
$(function() {
	...
	$('.open-wishlist-popup').on('mouseover', function(e){
		...
		if(!$('.wishlist-box.popup').hasClass('active')){
			closePopups();
			if($(this).offset().left>winW*0.5){
				$('.wishlist-box.popup').addClass('active wishlist-right').css({'left':'auto', 'right':winW - $(this).offset().left-$(this).outerWidth()*0.5-47, 'top':$(this).offset().top-winScr+15, 'opacity':'0'}).stop().animate({'opacity':'1'}, 300);			
			}
			else{
				$('.wishlist-box.popup').addClass('active wishlist-left').css({'right':'auto', 'left':$(this).offset().left, 'top':$(this).offset().top-winScr+15, 'opacity':'0'}).stop().animate({'opacity':'1'}, 300);			
			}
		}
	...
	$('.open-wishlist-popup').on('mouseleave', function(){
		closewishlistTimeout = setTimeout(function(){closePopups();}, 1000);
	});
	...
	$('.show-boxall').click(function(){
		$('.hide-boxall').show();
		$('.box-all').show();
		$('.owl-hrizon').hide();
		$('.show-boxall').hide();
	});
	$('.hide-boxall').click(function(){
		$('.show-boxall').show();
		$('.box-all').hide();
		$('.owl-hrizon').show();
		$('.hide-boxall').hide();
	...
	/* $('.close-popup').click(function(){
		$('.fade').removeClass('in');
		$('.modal-open').addClass('modal-open2');
	}); */
